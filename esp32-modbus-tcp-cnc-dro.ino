/**
 * =========================================================================================
 * Project     : ESP32 Modbus TCP CNC Digital Readout (DRO) & Axis Controller
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Industrial CNC multi-axis DRO gateway reading quadrature optical linear scales, calculating sub-micron tool offsets, and broadcasting real-time position via Modbus TCP & WebSockets.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-modbus-tcp-cnc-dro";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 Modbus TCP CNC Digital Readout (DRO) & Axis Controller"));
